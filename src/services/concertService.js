...
const getUserSe = () => {
  try {
    ...
    return Math.max(1, parseInt(userSe)); // 최소 1, 반드시 정수
  } catch (error) {
...
export const concertService = {
  ...
  // 특정 공연 조회 - 대기열 처리 포함
  async getConcertById(id, options = {}) {
    const { skipQueue = false, isRetry = false } = options;
    ...
    try {
      ...
      const url = new URL(`${API_BASE_URL}/concerts/${numericId}`);
      
      // 대기열을 건너뛰지 않는 경우 user_se 추가
      if (!skipQueue) {
        url.searchParams.set('user_se', userSe);
      }

      console.log('공연 상세 요청:', url.toString());
      ...
      if (response.status === 425) {
        ...
        console.log('대기열 응답:', errorData);
        
        // 대기열 정보와 함께 특별한 에러 던지기
        const queueError = new Error('QUEUE_REQUIRED');
        queueError.queueInfo = {
          rank: errorData.detail?.rank || -1,
          total: errorData.detail?.total || 0,
          concertId: numericId,
          userSe: userSe
        };
        throw queueError;
      ...
      if (concert.seats) {
        try {
          const seatsList = typeof concert.seats === 'string' ? JSON.parse(concert.seats) : concert.seats;
      ...
      if (seatInitializationNeeded || Object.keys(seats).length === 0) {
        ...
        // 재귀 호출 방지를 위해 플래그 추가
        if (!isRetry) {
          return this.getConcertById(id, { ...options, isRetry: true });
        }
      }
  ...
  // 대기열에 등록
  async enqueueForConcert(concertId) {
    try {
      const userSe = getUserSe();
  ...
  // 대기열 상태 확인
  async getQueueStatus(concertId) {
    try {
      const userSe = getUserSe();
      const url = new URL(`${API_BASE_URL}/queue/status`);
      url.searchParams.set('concert_se', concertId.toString());
      url.searchParams.set('user_se', userSe);
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.detail || '대기열 상태를 확인할 수 없습니다.');
      }

      const result = await response.json();
      console.log('대기열 상태:', result);
  ...
  // 대기열 없이 공연 정보 조회 (관리자용)
  async getConcertByIdWithoutQueue(id) {
    return this.getConcertById(id, { skipQueue: true });
  },
...
export const queueService = {
  // 대기열 등록
  async enqueue(concertId) {
    return concertService.enqueueForConcert(concertId);
  },

  // 대기열 상태 확인
  async getStatus(concertId) {
    return concertService.getQueueStatus(concertId);
  },

  // SSE 연결을 위한 URL 생성
  getSSEUrl(concertId, userSe = null) {
    const actualUserSe = userSe || getUserSe();
    const url = new URL(`${API_BASE_URL}/queue/stream`);
    url.searchParams.set('concert_se', concertId.toString());
    url.searchParams.set('user_se', actualUserSe.toString());
    return url.toString();
  }
};
